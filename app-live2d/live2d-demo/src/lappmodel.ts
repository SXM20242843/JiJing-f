...
export class LAppModel extends CubismUserModel {
   ...
   * @param fileName
   */
  private _useExternalMouthPose = false;
  private _externalMouthOpenY = 0;
  private _externalMouthForm = 0;
  private _talkPhase = 0;
  
  public setExternalMouthPose(openY: number, form: number): void {
    this._externalMouthOpenY = Math.max(0, Math.min(1, openY));
    this._externalMouthForm = Math.max(-1, Math.min(1, form));
    this._useExternalMouthPose = true;
  }
  
  public clearExternalMouthPose(): void {
    this._externalMouthOpenY = 0;
    this._externalMouthForm = 0;
    this._useExternalMouthPose = false;
  }
  
  private applyExternalMouthPose(deltaTimeSeconds: number): void {
    if (!this._model || !this._useExternalMouthPose) return;
  
    const mouthOpenId = CubismFramework.getIdManager().getId('PARAM_MOUTH_OPEN_Y');
    const mouthFormId = CubismFramework.getIdManager().getId('PARAM_MOUTH_FORM');
  
    const angleXId = CubismFramework.getIdManager().getId('PARAM_ANGLE_X');
    const angleYId = CubismFramework.getIdManager().getId('PARAM_ANGLE_Y');
    const bodyAngleXId = CubismFramework.getIdManager().getId('PARAM_BODY_ANGLE_X');
    const breathId = CubismFramework.getIdManager().getId('PARAM_BREATH');
  
    this._model.setParameterValueById(mouthOpenId, this._externalMouthOpenY, 1.0);
    this._model.setParameterValueById(mouthFormId, this._externalMouthForm, 1.0);
  
    // 轻微点头 + 呼吸，增强“正在讲解”的感觉
    const talkPower = this._externalMouthOpenY;
    this._talkPhase += deltaTimeSeconds * (1.8 + talkPower * 2.2);
  
    const headX = Math.sin(this._talkPhase * 0.9) * talkPower * 0.35;
    const headY = Math.sin(this._talkPhase * 1.4) * talkPower * 0.12;
    const bodyX = Math.sin(this._talkPhase * 0.8) * talkPower * 0.10;
    const breath = 0.06 + talkPower * 0.10;
  
    this._model.addParameterValueById(angleXId, headX, 1.0);
    this._model.addParameterValueById(angleYId, headY, 1.0);
    this._model.addParameterValueById(bodyAngleXId, bodyX, 1.0);
    this._model.addParameterValueById(breathId, breath, 1.0);
  }
  ...
  public update(): void {
    ...
    // 先应用旧的外部 lipsync（兼容你现在已有逻辑）
    if (this._useExternalLipSync && this._lipSyncIds.length > 0) {
      this._model.setParameterValueById(
        this._lipSyncIds[0],
        this._externalLipSyncValue
      );
    }
  
    // 再应用新的双参数嘴型，覆盖旧的单参数效果
    this.applyExternalMouthPose(deltaTimeSeconds);
  ...
  public setExternalLipSyncValue(value: number): void {
    ...
    this._externalLipSyncValue = this._externalLipSyncValue * 0.7 + v * 0.3;
    this._useExternalLipSync = true;
  }
  
  public clearExternalLipSync(): void {
    this._externalLipSyncValue = 0;
    this._useExternalLipSync = false;
  }
  ...
  _externalLipSyncValue: number; // 外部注入的嘴型值 0~1
  _useExternalLipSync: boolean; // 是否启用外部嘴型
}
