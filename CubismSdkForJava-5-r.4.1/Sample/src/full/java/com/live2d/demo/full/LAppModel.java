...
public class LAppModel extends CubismUserModel {
    ...
    public void update() {
        ...
        // AI mouthFrames Lip Sync
        float mouthOpen = MouthSyncManager.getInstance().getCurrentMouthOpen();
        float mouthForm = MouthSyncManager.getInstance().getCurrentMouthForm();

        // 后端 mouthFrames 的 open 值偏小，这里放大一点，让演示时嘴型更明显
        mouthOpen = mouthOpen * 3.5f;
        ...
        // 优先使用模型自身定义的 LipSync 参数
        if (lipSyncIds != null && lipSyncIds.size() > 0) {
            for (int i = 0; i < lipSyncIds.size(); i++) {
                CubismId lipSyncId = lipSyncIds.get(i);
                model.addParameterValue(lipSyncId, mouthOpen, 1.0f);
            }
        }
        ...
        // 如果模型有嘴型形状参数，也一起写；没有这个参数一般不会影响运行
    ...
    private void applyClothesModeInternal() {
        if (model == null) {
            return;
        }

        if ("uniform".equals(clothesMode)) {
            // 先保守处理：只切身体，不强制显示 A/B 手臂，避免四条胳膊
            setPartOpacitySafely(HARU_DRESS_BODY, 0.0f);
            setPartOpacitySafely(HARU_UNIFORM_BODY, 1.0f);
        ...
        if ("dress".equals(clothesMode)) {
            // 连衣裙身体显示
            setPartOpacitySafely(HARU_DRESS_BODY, 1.0f);
            setPartOpacitySafely(HARU_UNIFORM_BODY, 0.0f);
    ...
    private void setPartOpacities(String[] partIds, float opacity) {
        if (partIds == null || model == null) {
            return;
        }

        for (String partId : partIds) {
            setPartOpacitySafely(partId, opacity);
        }
    }

    private void setPartOpacitySafely(String partId, float opacity) {
        if (partId == null || partId.trim().length() == 0 || model == null) {
            return;
        }
    ...
    // haru 身体部件
    private static final String HARU_DRESS_BODY = "PARTS_01_BODY_001";
    private static final String HARU_UNIFORM_BODY = "PARTS_01_BODY_002";

    // haru 连衣裙手臂部件：A / B 是不同姿势，不可以同时强制显示
    private static final String[] HARU_DRESS_ARMS = {
            "PARTS_01_ARM_R_A_001",
            "PARTS_01_ARM_L_A_001",
            "PARTS_01_ARM_R_B_001",
            "PARTS_01_ARM_L_B_001"
    };

    // haru 制服手臂部件：目前先隐藏，避免和连衣裙手臂叠出 4 条胳膊
    private static final String[] HARU_UNIFORM_ARMS = {
